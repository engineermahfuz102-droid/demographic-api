...
async def seed_db():
    ...
    try:
        ...
        # Use Railway working directory
        BASE_DIR = os.getcwd()
        json_path = os.path.join(BASE_DIR, "profiles.json")

        print(f"🔍 Looking for profiles.json at: {json_path}")
        print(f"📂 Files in directory: {os.listdir(BASE_DIR)}")

        if not os.path.exists(json_path):
            print("❌ profiles.json NOT FOUND — seeding aborted")
            return

        print(f"✅ Found profiles.json at: {json_path}")
        ...
        for item in data:
            exists = db.query(Profile).filter_by(name=item["name"]).first()
            if exists:
                continue
        ...
        db.commit()
        print(f"✅ Seeded {inserted} records successfully")
