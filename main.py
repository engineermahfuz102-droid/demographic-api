...
Index('ix_performance', Profile.gender, Profile.age, Profile.country_id)
...
def parse_natural_query(q: str):
    ...
    return filters if matched else None
...
async def seed_db():
    db = SessionLocal()
    try:
        if db.query(Profile).count() == 0 and os.path.exists("profiles.json"):
            with open("profiles.json", "r") as f:
                data = json.load(f)
            if isinstance(data, dict): data = data.get("data", data.get("records", []))
            if data:
...
async def create_profile(request: Request):
    ...
    try:
        ...
        valid_data = {k: v for k, v in data.items() if hasattr(Profile, k)}
        if "id" not in valid_data: valid_data["id"] = str(uuid6.uuid7())
        new_p = Profile(**valid_data)
        db.add(new_p)
        db.commit()
        db.refresh(new_p)
        return {"status": "success", "data": new_p}
